...
# A sorted array of distinct integers shiftArr is shifted to the left
...
# Explain your solution and analyze
# its time and space complexities. Solution should be in O(log N)

def get_index(shifted_array, number):
    pivot_index = find_pivot(shifted_array, 0, len(shifted_array))
    pivot_value = shifted_array[pivot_index]

    if pivot_value == number:
        return pivot_index
    elif number < shifted_array[0]:
        return binary_search(shifted_array, number, pivot_index + 1, len(shifted_array))
    else:
        return binary_search(shifted_array, number, 0, pivot_index)

def find_pivot(shifted_array, start, end):
    midpoint = (start + end) // 2
    value = shifted_array[midpoint]
    if value > shifted_array[midpoint + 1]:
        return midpoint
    elif value > shifted_array[-1]:
        return find_pivot(shifted_array, midpoint, end)
    else:
        return find_pivot(shifted_array, start, midpoint)

def binary_search(shifted_array, num_to_find, start, end):
    midpoint = (start + end) // 2
    value = shifted_array[midpoint]
    if value == num_to_find:
        return midpoint
    elif value < num_to_find:
        return binary_search(shifted_array, num_to_find, midpoint + 1, end)
    else:
        return binary_search(shifted_array, num_to_find, start, midpoint)
